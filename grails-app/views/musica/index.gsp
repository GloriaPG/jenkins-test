		...
		<div id="list-musica" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="banda" title="${message(code: 'musica.banda.label', default: 'Banda')}" />
					
						<g:sortableColumn property="nombre" title="${message(code: 'musica.nombre.label', default: 'Nombre')}" />
				...
				<g:each in="${musicaInstanceList}" status="i" var="musicaInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${musicaInstance.id}">${fieldValue(bean: musicaInstance, field: "banda")}</g:link></td>
					
						<td>${fieldValue(bean: musicaInstance, field: "nombre")}</td>
